...
namespace TicketService.Services
{
    ...
    public sealed class TicketWorkflowService(ITicketRepository ticketRepository, IEventPublisher eventPublisher, IPolicyValidationService policyValidationService) : ITicketWorkflowService
    {
        ...
        {
            if (!string.Equals(user.Role, "Customer", StringComparison.OrdinalIgnoreCase)) throw new UnauthorizedAccessException("Only customers can create tickets.");
            if (request.Type == TicketType.Claim && request.ClaimAmount is null) throw new InvalidOperationException("Claim amount is required for claim tickets.");
            ...
            var ticket = new Ticket
            {
                ...
                ClaimDetails = request.Type == TicketType.Claim ? new ClaimDetails { ClaimAmount = request.ClaimAmount ?? 0, Documents = request.Documents ?? string.Empty } : null
            };
        ...
        public async Task<IReadOnlyList<Ticket>> GetAsync(CurrentUser user, CancellationToken cancellationToken)
        {
            ...
            return user.Role switch
            {
                ...
                "ClaimsSpecialist" => tickets.Where(x => x.Type == TicketType.Claim).ToList(),
                "SupportSpecialist" => tickets.Where(x => x.Type == TicketType.Support).ToList(),
                _ => tickets
            };
        }
        ...
        {
            if (!string.Equals(user.Role, "Admin", StringComparison.OrdinalIgnoreCase)) throw new UnauthorizedAccessException("Only admins can assign tickets.");
            var ticket = await RequireTicket(ticketId, cancellationToken);
        ...
        public Task<Ticket> ApproveClaimAsync(Guid ticketId, CurrentUser user, CancellationToken cancellationToken) => DecideClaimAsync(ticketId, ClaimApprovalStatus.Approved, TicketStatus.Resolved, "ClaimApproved", user, cancellationToken);
        public Task<Ticket> RejectClaimAsync(Guid ticketId, CurrentUser user, CancellationToken cancellationToken) => DecideClaimAsync(ticketId, ClaimApprovalStatus.Rejected, TicketStatus.Rejected, "ClaimRejected", user, cancellationToken);
        ...
        {
            var ticket = await RequireTicket(ticketId, cancellationToken);
            if (ticket.Type != TicketType.Claim || ticket.ClaimDetails is null) throw new InvalidOperationException("This ticket is not a claim.");
            if (!string.Equals(user.Role, "ClaimsSpecialist", StringComparison.OrdinalIgnoreCase)) throw new UnauthorizedAccessException("Only claims specialists can approve or reject claims.");
            EnsureAssignmentForStatusChange(ticket, user);
        ...
        private async Task<Ticket> RequireTicket(Guid ticketId, CancellationToken cancellationToken) => await ticketRepository.GetAsync(ticketId, cancellationToken) ?? throw new KeyNotFoundException("Ticket not found.");
        private static void EnsureAssignmentForStatusChange(Ticket ticket, CurrentUser user)
        {
            ...
            if (ticket.AssignedTo is null) throw new InvalidOperationException("Ticket must be assigned before its status can be changed.");
            if (ticket.AssignedTo != user.UserId) throw new UnauthorizedAccessException("Only the assigned user can change this ticket's status.");
        }
