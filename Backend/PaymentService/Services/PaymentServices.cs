...
namespace PaymentService.Services
{
    ...
    public sealed class PaymentService(IPaymentRepository repository, IEventPublisher eventPublisher) : IPaymentService
    {
        ...
        {
            // Deduplication: if a payment with this reference already exists (e.g. Razorpay already recorded it),
            // skip creating a duplicate and just return the existing one.
            if (!string.IsNullOrWhiteSpace(request.PaymentReference)
                && await repository.ExistsByReferenceAsync(request.PaymentReference, cancellationToken))
            {
                // Payment already recorded — publish PaymentCompleted so PolicyPaymentConsumer activates the policy
                await eventPublisher.PublishAsync("PaymentCompleted", new
                {
                    PaymentId = Guid.Empty,
                    request.CustomerId,
                    request.PolicyId,
                    request.CustomerPolicyId,
                    request.Amount,
        ...
        public Task<IReadOnlyList<Payment>> GetMyAsync(Guid customerId, CancellationToken cancellationToken)
            => repository.GetByCustomerIdAsync(customerId, cancellationToken).ContinueWith<IReadOnlyList<Payment>>(t => t.Result, cancellationToken);
