...
public sealed class TicketsController(ITicketWorkflowService ticketWorkflowService) : ControllerBase
{
    ...
    public async Task<ActionResult> GetById(Guid ticketId, CancellationToken cancellationToken)
    {
        var ticket = await ticketWorkflowService.GetByIdAsync(ticketId, User.ToCurrentUser(), cancellationToken);
