...
namespace TicketService.Services
{
    ...
    public sealed class TicketWorkflowService(ITicketRepository ticketRepository, IEventPublisher eventPublisher, IPolicyValidationService policyValidationService) : ITicketWorkflowService
    {
        public async Task<Ticket> CreateAsync(CreateTicketRequest request, CurrentUser user, CancellationToken cancellationToken)
        {
            ...
            var ticket = new Ticket
            {
                ...
                ClaimDetails = request.Type == TicketType.Claim
                    ? new ClaimDetails
                    {
                        ClaimAmount = request.ClaimAmount ?? 0,
                        Documents = request.DocumentFileName ?? string.Empty
                    }
                    : null
            };
        ...
        public async Task<Ticket?> GetByIdAsync(Guid ticketId, CurrentUser user, CancellationToken cancellationToken)
        {
            var ticket = await ticketRepository.GetAsync(ticketId, cancellationToken);
            if (ticket is null) return null;

            // Enforce same access rules as the list endpoint
            var canAccess = user.Role switch
            {
                "Admin" => true,
                "Customer" => ticket.CustomerId == user.UserId,
                "ClaimsSpecialist" => ticket.Type == TicketType.Claim,
                "SupportSpecialist" => ticket.Type == TicketType.Support,
                _ => false
            };

            return canAccess ? ticket : null;
        }
