...
namespace NotificationService.Repositories
{
    public sealed class NotificationRepository(NotificationDbContext dbContext) : INotificationRepository
    {
        ...
        {
            // Admin sees: their own notifications (UserId == adminId) + platform-wide alerts (UserId == null)
            // Other roles see: only their own notifications (UserId == userId)
            var query = isAdmin
                ? dbContext.Notifications.Where(x => x.UserId == userId || x.UserId == null)
                : dbContext.Notifications.Where(x => x.UserId == userId);
