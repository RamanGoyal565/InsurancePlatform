            ...
            foreach (var audit in audits.OrderBy(x => x.OccurredOnUtc))
            {
                ...
                switch (audit.EventType)
                {
                    case "PolicyPurchased":
                        ...
                        break;
                    // PaymentCompleted also signals a successful purchase/renewal — use as fallback
                    case "PaymentCompleted":
                        var operation = GetString(payload, "Operation") ?? string.Empty;
                        if (string.Equals(operation, "Purchase", StringComparison.OrdinalIgnoreCase))
                        {
                            existing = existing with
                            {
                                PolicyId = GetGuid(payload, "PolicyId") ?? existing.PolicyId,
                                CustomerId = GetGuid(payload, "CustomerId") ?? existing.CustomerId,
                                Status = existing.Status == "Unknown" ? "Active" : existing.Status,
                                PurchasedAtUtc = existing.Status == "Unknown" ? audit.OccurredOnUtc : existing.PurchasedAtUtc
                            };
                        }
                        else if (string.Equals(operation, "Renewal", StringComparison.OrdinalIgnoreCase))
                        {
                            existing = existing with
                            {
                                Status = "Renewed",
